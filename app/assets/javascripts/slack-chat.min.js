...
(console.log("[SlackChat] Query failed with errors: "),console.log(a))}})})},postMessage:function(a){a=a._options;var e={};e.fallback="View "+a.user+"'s profile";e.color=a.slackColor;e.author_name=a.user;""!==a.userLink&&(e.author_link=a.userLink);""!==a.userImg&&(e.author_icon=a.userImg);""!==a.userId&&(e.fields=[{value:a.userId,"short":!0}]);if(""===c(".slack-new-message").val().trim())return!1;message=c(".slack-new-message").val();c(".slack-new-message").val("");a.debug&&(console.log("Posting Message:"),
