class EmailValidationMailer < ApplicationMailer
  ...
  def notice_google_apps(user)
    ...
    not_broken_redirection = @user.email_redirect_accounts.where(type_redir: "smtp").select{|e| true unless e.broken?}.map(&:redirect)

    not_broken_redirection.each do |email|
