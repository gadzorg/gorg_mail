class Ml::ListsController < ApplicationController
  ...
  def show
    ...
    @search = params[:search]
    @members_collection = (@search.present? ? @ml_list.all_members.search(@search) : User.none).order(:firstname).page(params[:page]).per_page(20).where(email_source_accounts: {primary: true})
