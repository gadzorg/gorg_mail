...
RSpec.describe SetupSubscriptionService, type: :service do
  ...
  describe '#mailing_lists' do
    before :each do
      ...
      emails.each do |email|
        create(:ml_list, name: email, email: email)
      end
    end

    context "for a gadz after 2000" do
      let(:user) { create(:user, gadz_centre_principal: "bo", gadz_proms_principale: "2017") }
    ...
    context "for a gadz before 2000" do
      let(:user) { create(:user, gadz_centre_principal: "bo", gadz_proms_principale: "1998") }
      let(:service) { SetupSubscriptionService.new(user) }
    ...
    context "for a gadz of Aix(ai)" do
      let(:user) { create(:user, gadz_centre_principal: "ai", gadz_proms_principale: "2017") }
      let(:service) { SetupSubscriptionService.new(user) }
    ...
    context "for a non gadz" do
      let(:user) { create(:user, gadz_centre_principal: "", gadz_proms_principale: "") }
      let(:service) { SetupSubscriptionService.new(user) }
  ...
  describe '#do_subscribe' do
    let(:list) { create(:ml_list, email: 'foobar@gadz.org') }
  ...
  describe "#gadz_tabagns" do
    describe "for a gadz whose tabagns is not Aix" do
      let(:user) { create(:user, gadz_centre_principal: "bo") }
      let(:service) { SetupSubscriptionService.new(user) }
    ...
    describe "for a gadz of Aix" do
      let(:user) { create(:user, gadz_centre_principal: "ai") }
      let(:service) { SetupSubscriptionService.new(user) }
  ...
  describe "#gadz_ans" do
    describe "for a gadz after 2000" do
      let(:user) { create(:user, gadz_proms_principale: "2017") }
      let(:service) { SetupSubscriptionService.new(user) }
    ...
    describe "for a gadz before 2000" do
      let(:user) { create(:user, gadz_proms_principale: "1998") }
      let(:service) { SetupSubscriptionService.new(user) }
