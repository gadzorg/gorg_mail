...
RSpec.describe Alias, type: :model do
  ...
  describe "validation" do
    it "is not valid with an non existent id" do
      expect(build(:alias, email_virtual_domain_id: 157)).not_to be_valid
    end
  end

  describe 'search' do

    let!(:alias1) { create(:alias, email: "emailA1", redirect: "redirect1@exampleC.com") }
    let!(:alias2) { create(:alias, email: "emailA2", redirect: "redirect2@exampleD.com") }
    let!(:alias3) { create(:alias, email: "emailB3", redirect: "redirect3@exampleD.com") }
