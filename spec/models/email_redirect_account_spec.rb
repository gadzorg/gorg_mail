...
RSpec.describe EmailRedirectAccount, type: :model do
  ...
  it "has a valid Factory" do
    expect(build(:email_redirect_account)).to be_valid
  end
  ...
  describe "validations" do
    ...
    describe "does not accept internal email addresses as redirection" do
      it "does not validate" do
        expect(build(:email_redirect_account,redirect: "local@#{EmailVirtualDomain.first.name}")).not_to be_valid
      end

      it "add an error message" do
        record= build(:email_redirect_account,redirect: "local@#{EmailVirtualDomain.first.name}")
        record.valid?
    ...
    describe "validate occurencies of 'active flags'" do
      it "does not validate when their is more than #{Configurable.max_actives_era} ERA per user" do
        Configurable.max_actives_era.times{ create(:email_redirect_account,flag: 'active',type_redir: "smtp", user_id: 1)}
        expect(build(:email_redirect_account,flag: 'active',type_redir: "smtp", user_id: 1)).not_to be_valid
      end

      it "validates only on smtp" do
        Configurable.max_actives_era.times{ create(:email_redirect_account,flag: 'active',type_redir: "googleapps", user_id: 1)}
        expect(build(:email_redirect_account,flag: 'active',type_redir: "smtp", user_id: 1)).to be_valid
      end
    end
  end
end